...
- `src/*.html` are vanilla HTML/CSS/JS — no framework, no JSX. If a renderer needs a Node API, route it through an IPC handler in `main.ts` and expose it via `preload.ts`.
...
- New IPC handler → `main.ts`, add an `ipcMain.handle('domain-verb', ...)`, then expose in `preload.ts`.
